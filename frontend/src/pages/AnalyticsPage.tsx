...
export function AnalyticsPage() {
  ...
  const [curve, setCurve] = useState<{ day: number; retention: number }[]>([]);
  const [heatmap, setHeatmap] = useState<HeatmapDay[]>([]);
  const [recall, setRecall] = useState<PredictedRecall[]>([]);
  const [dailyQueue, setDailyQueue] = useState<DailyQueueCard[]>([]);
    ...
    <div className="p-8 space-y-8">
      ...
      <div style={cardStyle}>
        ...
        <div className="space-y-4">
          {recall
            ?.sort((a, b) => a.predicted_recall - b.predicted_recall)
            .slice(0, 5)
            .map((card) => (
              <div
                key={card.card_id}
